...
// page logic begins
////////////////////////////
phpLog($DEBUG,sprintf("read.php, param1[%s], param2[%s]", $myreq['from'], $myreq['to']));

if( $res = dbGetEvents($myreq['from'], $myreq['to'])) {
	phpLog($DEBUG,sprintf("read.php, events: [%s]", print_r($res,true)));
	$myres['read'] = 1;
	$myres['msg'] = 'read succ';
	$myres['events'] = $res;
}
